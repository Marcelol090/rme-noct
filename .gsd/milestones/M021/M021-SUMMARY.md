# M021 - SPR compressed payload reads summary

## Completed

- Added a pure Python compressed SPR payload reader.
- Read raw payload bytes from `SprFrameRecord.archive_offset`.
- Matched legacy `archive_offset + 3` seek behavior.
- Read little-endian `u16` compressed payload sizes.
- Returned exact raw compressed bytes without decoding RLE pixels.
- Returned explicit empty payload outcomes for sprite id `0` and offset `0`.
- Exported payload reader symbols from `pyrme.rendering`.
...
- `pyrme stack --quiet` is environment-blocked in WSL by Qt platform setup (`wayland` plugin load failure).

...
impl<'py> FromPyObject<'py> for SpriteData {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        let sprite = ob.downcast::<PyDict>().map_err(|_| {
            PyTypeError::new_err("sprite payload must be a mapping")
        })?;
        Ok(Self {
...
impl SpriteAtlas {
    ...
    pub fn map_buffer(&mut self, pixels: Vec<u8>) -> PyResult<bool> {
        validate_sprite_byte_len(pixels.len())?;
        self.mapped_pixels = pixels;
        self.mapped_sprite_count = 1;
        Ok(true)
    }

    pub fn mapped_pixels(&self) -> Vec<u8> {
        self.mapped_pixels.clone()
    }
    ...
    pub fn render_frame(&mut self, sprites: Vec<Bound<'_, PyAny>>) -> PyResult<usize> {
        let mut processed = 0;
        self.last_frame.clear();
        for item in sprites {
            let dict = item.downcast::<PyDict>()?;
            
            let x = extract_u32(dict, "x")?;
            let y = extract_u32(dict, "y")?;
            let layer = extract_u32(dict, "layer")?;
            let sprite_id = extract_u32(dict, "sprite_id")?;
            let pixels = extract_pixel_bytes(&extract_required(dict, "pixels")?)?;
            
            validate_sprite_byte_len(pixels.len())?;
            
            self.last_frame.push(SpriteFrameCommand {
                x, y, layer, sprite_id, pixels
            });
            processed += 1;
        }
        Ok(processed)
...
fn extract_required<'py>(
    sprite: &Bound<'py, PyDict>,
    key: &str,
...
fn extract_pixel_bytes(value: &Bound<'_, PyAny>) -> PyResult<Vec<u8>> {
    let bytes = value.downcast::<PyBytes>().map_err(|_| {
        PyTypeError::new_err("sprite payload field `pixels` must be bytes")
    })?;
